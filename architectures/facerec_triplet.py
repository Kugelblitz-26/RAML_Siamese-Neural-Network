...
FACE_DATA_PATH = '/home/charanz/Desktop/MASTERS/RAML/ref/facerec/data/Face Data/Face Dataset'
EXTRACTED_FACES_PATH = '/home/charanz/Desktop/MASTERS/RAML/ref/facerec/data/Extracted Faces/Extracted Faces'
...
DATASET = '/home/charanz/Desktop/MASTERS/RAML/ref/facerec/data/images/output_dataset'
...
class SiameseModel(Model):
    ...
    def train_step(self, data):
        # DEBUGGING: Print raw data input
        print("\n[DEBUG] Raw data received in train_step:")
        print(f"Type: {type(data)}")
        
        if isinstance(data, tuple):
            print(f"Length of tuple: {len(data)}")
            print(f"data[0] type: {type(data[0])}")
            print(f"data[1] (label): {data[1] if len(data) > 1 else 'N/A'}")
            data = data[0]  # Only take the actual inputs

        if isinstance(data, (list, tuple)):
            print(f"data length: {len(data)}")
            for i, tensor in enumerate(data):
                print(f"Input {i} shape: {tensor.shape if hasattr(tensor, 'shape') else 'N/A'}")
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=val_steps_per_epoch, desc='Validation') as pbar:
            for batch in batch_generator(val_triplets, batch_size=batch_size):
                logs = model.test_on_batch(batch)
                loss, accuracy = logs["loss"], logs["accuracy"]
